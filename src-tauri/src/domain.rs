...
pub fn compute_recommendation(now_ms: i64, state: &AppState) -> Option<Recommendation> {
    let mut recs: Vec<Recommendation> = state
        ...
        // Do not recommend the currently running task
        .filter(|t| t.status != Status::Active)
        // Exclude deleted from recs
...
pub fn compute_sorted_tasks(now_ms: i64, state: &AppState) -> Vec<Task> {
    let mut recs: Vec<(Task, i32)> = state
        .tasks
        .iter()
        .filter(|t| t.status != Status::Archived && t.status != Status::Deleted)
        .map(|t| {
...
// Pure: compute scored list for all tasks (filters archived), sorted like compute_sorted_tasks
pub fn compute_scored_tasks(now_ms: i64, state: &AppState) -> Vec<Recommendation> {
    let mut recs: Vec<Recommendation> = state
        .tasks
        .iter()
        .filter(|t| t.status != Status::Archived && t.status != Status::Deleted)
        ...
        a.task
            ...
            .then_with(|| {
                if !a.task.done && !b.task.done {
                    // Active queue: higher score first, tie by oldest touched, then earliest due
                    b.score
                        .cmp(&a.score)
                        .then_with(|| a.task.last_touched_ms.cmp(&b.task.last_touched_ms))
                        .then_with(|| a.task.due_ts.cmp(&b.task.due_ts))
                } else {
                    // Done section: group by first tag asc then title
                    let atag = a.task.tags.first().cloned().unwrap_or_default();
                    let btag = b.task.tags.first().cloned().unwrap_or_default();
                    atag.cmp(&btag).then_with(|| a.task.title.cmp(&b.task.title))
                }
            })
    });
